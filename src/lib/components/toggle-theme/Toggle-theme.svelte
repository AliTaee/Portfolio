<script lang="ts">
	...
	function switchTheme(e: Event) {
		...
		localStorage.setItem(LOCAL_STORAGE_KEY, target.checked ? NIGHT_KEY : LIGHT_KEY)
	}
</script>

<label class="swap swap-rotate cursor-pointer" for="night-theme">
	<input checked={nightTheme} type="checkbox" id="night-theme" on:change={switchTheme} />
	<!-- sun icon -->
	<svg
		class="swap-on h-10 w-10 fill-slate-100"
	...
	<!-- moon icon -->
	<svg
		class="swap-off h-10 w-10 fill-slate-100"
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		><path
